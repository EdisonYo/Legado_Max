...
> 在其他设置中启用，并且支持目录每页之间可以停顿一定时间后加载。打开此功能后，无法调试下一页目录，只调试第一页目录就会进入正文的调试，防止目录并发加载导致网站压力过大。而且开启此功能后，只要目录加载出一页，在详情中就能进行阅读或者查看目录。
...
> 通过 book.type 参数控制书籍 / 书源类型，原版只能在目录列表控制，经过lyc修改后正文规则也可以控制，对应数值含义
 4: 音频  
 8: 文本  
 16: 更新失败  
 32: 音频  
