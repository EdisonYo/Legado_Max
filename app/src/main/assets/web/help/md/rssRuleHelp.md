...
> 启用后加载网页会优先加载缓存内容，而不是从网络加载。如果缓存不存在才从网络加载。\
> **应用场景**：
> 1. RSS阅读界面：用户阅读文章正文时，WebView会优先加载缓存的网页内容。\
> 2. 规则执行：订阅源规则中使用WebView相关方法时（如 `webView()`、`webViewGetSource()`、`webViewGetOverrideUrl()`），也会优先使用缓存。\
> **实际效果**：
> 启用时：如果之前访问过相同URL，直接从缓存读取，加载速度快，节省流量。\
> 未启用时（默认）：按照标准HTTP缓存策略，由WebView根据缓存控制头决定是否使用缓存。
