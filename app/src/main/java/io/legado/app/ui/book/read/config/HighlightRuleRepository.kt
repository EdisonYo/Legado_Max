...
/**
 * 高亮规则存储管理（合并原 HighlightRuleStore + HighlightRuleGroupStore）
 */
object HighlightRuleRepository {
    ...
    fun load(context: Context): MutableList<HighlightRule> {
        ...
        if (rules != null) {
            ...
            if (normalized != rules) {
                ...
                ensureGroupsFromRules(context, normalized)
            }
    ...
    fun saveGroups(context: Context, groups: List<String>) {
        val normalized = groups.filter { it.isNotBlank() }.distinct()
            .ifEmpty { listOf(DEFAULT_GROUP) }
        context.putPrefString(PreferKey.highlightRuleGroups, GSON.toJson(normalized))
    }

    private fun ensureGroupsFromRules(context: Context, rules: List<HighlightRule>) {
        val groups = loadGroups(context)
        val merged = LinkedHashSet<String>()
        merged.addAll(groups)
        rules.mapTo(merged) { it.group.ifBlank { DEFAULT_GROUP } }
        saveGroups(context, merged.toList())
    }

    // ==================== 回收站 ====================

    fun recycleRules(context: Context, rules: List<HighlightRule>) {
        SourceRecycleBinHelp.recycleHighlightRules(rules)
    }
    ...
    fun restoreBackupData(
        ...
        saveGroups(context, backupData.groups)
    ...
    fun sanitizeRule(
        rule: HighlightRule,
        fallbackGroup: String = DEFAULT_GROUP,
    ): HighlightRule {
...
疑是地上霜。",
    ...
    private val legacyBuiltinPatterns = mapOf(
        ...
        "thought_default" to "（[^）]*?(想道|暗道|心道|心里|想着|思量|思尙|盘算|盘算着)[^）]*?）",
    ...
    private val garbledMarkers = listOf("锻", "鏫", "鎥", "瀕", "涔", "錍", "钪", "錎", "绗")
}
